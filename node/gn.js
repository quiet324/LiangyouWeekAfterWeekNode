    ...
    (function(err, results) {
        ...
        results.forEach(function(audio, arrayIndex) {
            ...
            download(audio.downUrl).then(data => {
                fs.writeFileSync('../gn/' + fileName, data);
            });
            audio.duration = 1760;
            audio.size = "14.1M";
            audio.artistId = 22;
            audio.artistName = "现代人的希望";
            audio.id = 22000001 + arrayIndex;

        });

        fs.writeFile("./gn.json", JSON.stringify(results, null, '\t'));
    });