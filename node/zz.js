    ...
    (function(err, results) {
        ...
        results.forEach(function(audio, arrayIndex) {
            ...
            download(audio.downUrl).then(data => {
                fs.writeFileSync('./zz/' + fileName, data);
            });
            audio.duration = 1760;
            audio.size = "14.1M";
            audio.artistId = 9;
            audio.artistName = "零点零距离";
            audio.id = 9000001 + arrayIndex;

        });

        fs.writeFile("./zz.json", JSON.stringify(results, null, '\t'));
    });