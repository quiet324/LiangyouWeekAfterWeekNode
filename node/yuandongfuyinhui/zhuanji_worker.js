...
const url = 'http://www.feearadio.net/official/mobile2/bible?gid=10';
...
x(url, '.maincontent .files tr', [{
    ...
    (function(err, hrefs) {
        ...
        hrefs.forEach(function(audio, arrayIndex) {
            ...
            audio.duration = 1400;
            audio.size = "11M";
            audio.albumName = "窝客（worker）你在做什么？";
            // audio.albumId = 6008;
            // audio.id = 6008888180 + arrayIndex;
            // audio.id = _.last(results).id + 1 + arrayIndex;

            audio.id = 6025 * 1000000 + arrayIndex + 1;
            audio.albumId = 6025;
            audio.albumtitle = "窝客（worker）你在做什么？" + "(" + (arrayIndex + 1) + ")";
        ...
        fs.writeFile("./zhuanji_worker.json", JSON.stringify(hrefs, null, '\t'));
