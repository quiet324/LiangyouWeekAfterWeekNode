...
const url = 'http://www.feearadio.net/official/mobile2/bible?tid=&gid=10&cid=448';
...
x(url, '.maincontent .files tr', [{
    ...
    (function(err, hrefs) {
        ...
        hrefs.forEach(function(audio, arrayIndex) {
            ...
            audio.duration = 1200;
            audio.size = "10M";
            audio.albumName = "动绝妙 - 重光哥";
            // audio.albumId = 6008;
            // audio.id = 6008888180 + arrayIndex;
            // audio.id = _.last(results).id + 1 + arrayIndex;

            audio.id = 6030 * 1000000 + arrayIndex + 1;
            audio.albumId = 6030;
            audio.albumtitle = "动绝妙 - 重光哥" + "(" + (arrayIndex + 1) + ")";
        ...
        fs.writeFile("./zhuanji_sports.json", JSON.stringify(hrefs, null, '\t'));
