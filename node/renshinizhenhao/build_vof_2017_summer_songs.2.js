...
var names = JSON.parse(fs.readFileSync('./vof2017_summer_names_with_dates.json', 'utf8'));
names.forEach(function(name, indexId) {
    var audio = {};
    audio.duration = 3500;
    audio.size = "26M";
    audio.path = "http://media.febcchinese.org/streaming/vof" + '/' + name.name;
    audio.id = 6350 * 1000000 + indexId + 1;
    audio.albumId = 6350;
    audio.albumName = "《认识你真好》2017年夏季合集";
    audio.albumtitle = "《认识你真好》2017年夏季合集" + "(" + (indexId + 1) + ")";
...
fs.writeFile("./vof_2017_summer_songs.json", JSON.stringify(audios, null, '\t'));
