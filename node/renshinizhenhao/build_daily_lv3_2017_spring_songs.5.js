...
var names = JSON.parse(fs.readFileSync('./daily_lv3_2017_spring_names_with_dates.json', 'utf8'));
names.forEach(function(name, indexId) {
    ...
    audio.path = "http://media.haomuren.org/dev/lv3/" + name.name;
    audio.id = 6365 * 1000000 + indexId + 1;
    audio.albumId = 6365;
    audio.albumName = "《活水（三）》春季合集";
    audio.albumtitle = "《活水（三）》春季合集" + "(" + (indexId + 1) + ")";
    audio.title = "活水（三）-" + name.dateZH;
...
fs.writeFile("./daily_lv3_2017_spring_songs.json", JSON.stringify(audios, null, '\t'));
