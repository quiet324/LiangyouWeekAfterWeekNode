...
var names = JSON.parse(fs.readFileSync('./daily_lv2017_summer_names_with_dates.json', 'utf8'));
names.forEach(function(name, indexId) {
    var audio = {};
    audio.duration = 300;
    audio.size = "1M";
    // http://media.haomuren.org/dev/lv/lv0101.mp3
    audio.path = "http://media.haomuren.org/dev/lv/" + name.name;
    audio.id = 6358 * 1000000 + indexId + 1;
    audio.albumId = 6358;
    audio.albumName = "《活水》夏季合集";
    audio.albumtitle = "《活水》夏季合集" + "(" + (indexId + 1) + ")";
    audio.title = "活水-" + name.dateZH;
...
fs.writeFile("./daily_lv_2017_summer_songs.json", JSON.stringify(audios, null, '\t'));
