...
var names = JSON.parse(fs.readFileSync('./daily_lv2_2017_spring_names_with_dates.json', 'utf8'));
names.forEach(function(name, indexId) {
    ...
    audio.path = "http://media.haomuren.org/dev/lv2/" + name.name;
    audio.id = 6361 * 1000000 + indexId + 1;
    audio.albumId = 6361;
    audio.albumName = "《活水（二）》春季合集";
    audio.albumtitle = "《活水（二）》春季合集" + "(" + (indexId + 1) + ")";
    audio.title = "活水（二）-" + name.dateZH;
...
fs.writeFile("./daily_lv2_2017_spring_songs.json", JSON.stringify(audios, null, '\t'));
