...
var names = JSON.parse(fs.readFileSync('./daily_lv3_2017_summer_names_with_dates.json', 'utf8'));
names.forEach(function(name, indexId) {
    ...
    audio.id = 6366 * 1000000 + indexId + 1;
    audio.albumId = 6366;
    audio.albumName = "《活水（三）》夏季合集";
    audio.albumtitle = "《活水（三）》夏季合集" + "(" + (indexId + 1) + ")";
...
fs.writeFile("./daily_lv3_2017_summer_songs.json", JSON.stringify(audios, null, '\t'));
