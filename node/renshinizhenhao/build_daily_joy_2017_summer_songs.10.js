...
var names = JSON.parse(fs.readFileSync('./daily_joy_2017_summer_names_with_dates.json', 'utf8'));
names.forEach(function(name, indexId) {
    ...
    audio.path = "http://media.haomuren.org/dev/Joy/" + name.name;
    audio.id = 6370 * 1000000 + indexId + 1;
    audio.albumId = 6370;
    audio.albumName = "《喜乐的心》夏季合集";
    audio.albumtitle = "《喜乐的心》夏季合集" + "(" + (indexId + 1) + ")";
    audio.title = "喜乐的心-" + name.dateZH;
...
fs.writeFile("./daily_joy_2017_summer_songs.json", JSON.stringify(audios, null, '\t'));
