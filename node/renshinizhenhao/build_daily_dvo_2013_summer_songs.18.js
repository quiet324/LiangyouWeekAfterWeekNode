...
var names = JSON.parse(fs.readFileSync('./daily_dvo_2013_summer_names_with_dates.json', 'utf8'));
names.forEach(function(name, indexId) {
    ...
    audio.path = "http://media.haomuren.org/devotion/siD_mc/" + name.dateMonth + "/" + name.name;
    audio.id = 6378 * 1000000 + indexId + 1;
    audio.albumId = 6378;
    audio.albumName = "《荒漠甘泉乐侶》夏季合集";
    audio.albumtitle = "《荒漠甘泉乐侶》夏季合集" + "(" + (indexId + 1) + ")";
    audio.title = "荒漠甘泉乐侶-" + name.dateZH;
...
fs.writeFile("./daily_dvo_2013_summer_songs.json", JSON.stringify(audios, null, '\t'));
