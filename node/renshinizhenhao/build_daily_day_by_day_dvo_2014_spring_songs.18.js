...
var names = JSON.parse(fs.readFileSync('./daily_day_by_day_Dvo_2014_spring_names_with_dates.json', 'utf8'));
names.forEach(function(name, indexId) {
    ...
    audio.path = "http://media.haomuren.org/Devotion/DayByDay/" + name.dateMonth + "/" + name.name;
    audio.id = 6381 * 1000000 + indexId + 1;
    audio.albumId = 6381;
    audio.albumName = "《每日经历神》春季合集";
    audio.albumtitle = "《每日经历神》春季合集" + "(" + (indexId + 1) + ")";
    audio.title = "每日经历神-" + name.dateZH;
...
fs.writeFile("./daily_day_by_day_dvo_2014_spring_songs.json", JSON.stringify(audios, null, '\t'));
