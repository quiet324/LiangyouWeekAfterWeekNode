...
var names = JSON.parse(fs.readFileSync('./daily_utmost_Dvo_2016_winter_names_with_dates.json', 'utf8'));
names.forEach(function(name, indexId) {
    ...
    // http://media.haomuren.org/Devotion/Utmost/Dec/Dvo161213.mp3


    audio.path = "http://media.haomuren.org/Devotion/Utmost/" + name.dateMonth + "/" + name.name;
    audio.id = 6392 * 1000000 + indexId + 1;
    audio.albumId = 6392;
    audio.albumName = "《竭诚为主》冬季合集";
    audio.albumtitle = "《竭诚为主》冬季合集" + "(" + (indexId + 1) + ")";
    audio.title = "竭诚为主-" + name.dateZH;
...
fs.writeFile("./daily_utmost_dvo_2016_winter_songs.json", JSON.stringify(audios, null, '\t'));
