...
var names = JSON.parse(fs.readFileSync('./daily_ev_2017_spring_names_with_dates.json', 'utf8'));
names.forEach(function(name, indexId) {
    ...
    audio.path = "http://media.haomuren.org/dev/ev/" + name.name;
    audio.id = 6373 * 1000000 + indexId + 1;
    audio.albumId = 6373;
    audio.albumName = "《靜夜亮光》春季合集";
    audio.albumtitle = "《靜夜亮光》春季合集" + "(" + (indexId + 1) + ")";
    audio.title = "靜夜亮光-" + name.dateZH;
...
fs.writeFile("./daily_ev_2017_spring_songs.json", JSON.stringify(audios, null, '\t'));
