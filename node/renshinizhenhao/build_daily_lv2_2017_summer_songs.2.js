...
var names = JSON.parse(fs.readFileSync('./daily_lv2_2017_summer_names_with_dates.json', 'utf8'));
names.forEach(function(name, indexId) {
    ...
    audio.id = 6362 * 1000000 + indexId + 1;
    audio.albumId = 6362;
    audio.albumName = "《活水（二）》夏季合集";
    audio.albumtitle = "《活水（二）》夏季合集" + "(" + (indexId + 1) + ")";
...
fs.writeFile("./daily_lv2_2017_summer_songs.json", JSON.stringify(audios, null, '\t'));
