...
var names = JSON.parse(fs.readFileSync('./ib2017_names_with_dates.json', 'utf8'));
names.forEach(function(name, indexId) {
    var audio = {};
    audio.duration = 1760;
    audio.size = "14.1M";
    audio.path = "https://rawcdn.githack.com/quiet324/LiangYouResourceIB2017/170819/ib2017" + '/' + name.name;
    audio.id = 5100 * 1000000 + indexId + 1;
    audio.albumId = 5100;
    audio.albumName = "《无限飞行号》2017 上半年合集";
    audio.albumtitle = "《无限飞行号》2017 上半年合集" + "(" + (indexId + 1) + ")";
    audio.title = "无限飞行号-" + name.date;
...
fs.writeFile("./ib2017_songs.json", JSON.stringify(audios, null, '\t'));
