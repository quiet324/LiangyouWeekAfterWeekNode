    ...
    (function(err, results) {
        ...
        results.forEach(function(audio, arrayIndex) {
            ...
            download(audio.downUrl).then(data => {
                fs.writeFileSync('../se/' + fileName, data);
            });
            audio.duration = 1760;
            audio.size = "14.1M";
            audio.artistId = 19;
            audio.artistName = "恋爱季节";
            audio.id = 19000001 + arrayIndex;

        });

        fs.writeFile("./se.json", JSON.stringify(results, null, '\t'));
    });