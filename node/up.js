    ...
    (function(err, results) {
        ...
        results.forEach(function(audio, arrayIndex) {
            ...
            download(audio.downUrl).then(data => {
                fs.writeFileSync('../up/' + fileName, data);
            });
            audio.duration = 1760;
            audio.size = "14.1M";
            audio.artistId = 20;
            audio.artistName = "亲情不断电";
            audio.id = 20000001 + arrayIndex;

        });

        fs.writeFile("./up.json", JSON.stringify(results, null, '\t'));
    });