    ...
    (function(err, results) {
        ...
        results.forEach(function(audio, arrayIndex) {
            ...
            download(audio.downUrl).then(data => {
                fs.writeFileSync('../tm/' + fileName, data);
            });
            audio.duration = 1760;
            audio.size = "14.1M";
            audio.artistId = 5;
            audio.artistName = "我们仨，还有你";
            audio.id = 5000001 + arrayIndex;

        });

        fs.writeFile("./tm.json", JSON.stringify(results, null, '\t'));
    });