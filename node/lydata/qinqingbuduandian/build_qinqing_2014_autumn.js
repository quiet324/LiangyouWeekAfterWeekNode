...
datesEnglish.forEach(function(date, indexId) {
    ...
    x(realUrl, '.entry-post p', [{
        ...
        (function(err, hrefs) {
            ...
            audio.duration = 1760;
            audio.size = "14.1M";
            audio.albumName = "亲情不断电 " + year + "年秋季合集";
            audio.id = 6253 * 1000000 + (indexId + 1);
            audio.albumId = 6253;
            audio.albumtitle = "亲情不断电 " + year + "年秋季合集" + "(" + (indexId + 1) + ")";
    ...
    fs.writeFile("qinqing_" + year + "_autumn.json", JSON.stringify(audios, null, '\t'));
});