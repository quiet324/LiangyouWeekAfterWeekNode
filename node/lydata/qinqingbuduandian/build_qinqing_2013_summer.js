...
datesEnglish.forEach(function(date, indexId) {
    ...
    x(realUrl, '.entry-post font', [{
        ...
        (function(err, hrefs) {
            ...
            audio.duration = 1760;
            audio.size = "14.1M";
            audio.albumName = "亲情不断电 " + year + "年夏季合集";
            audio.id = 6248 * 1000000 + (indexId + 1);
            audio.albumId = 6248;
            audio.albumtitle = "亲情不断电 " + year + "年夏季合集" + "(" + (indexId + 1) + ")";
    ...
    fs.writeFile("qinqing_" + year + "_summer.json", JSON.stringify(audios, null, '\t'));
});