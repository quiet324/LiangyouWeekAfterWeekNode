...
for (var i = 1; i < 29; i++) {
    ...
    if (i < 10) {
        var realUrl = 'http://lydata.febcmedia.net/2013/02/0' + i + '/?arcf=cat:24';
    } else {
        var realUrl = 'http://lydata.febcmedia.net/2013/02/' + i + '/?arcf=cat:24';
    }
    x(realUrl, '.entry-post p', [{
        ...
        (function(err, hrefs) {
            ...
            if (i < 10) {
                ...
                audio.path = 'http://media.febcchinese.org/streaming/up/up13020' + i + '.mp3';
            } else {
                audio.title = hrefs[0].content + ' 201302' + i;
                audio.path = 'http://media.febcchinese.org/streaming/up/up1302' + i + '.mp3';
            }
            ...
            audio.id = 6247 * 1000000 + i + 31;
            audio.albumId = 6247;
            audio.albumtitle = "亲情不断电 2013年春季合集" + "(" + (i + 31) + ")";
    ...
    fs.writeFile("qinqing_2013_2.json", JSON.stringify(audios, null, '\t'));
}