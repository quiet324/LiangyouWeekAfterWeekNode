...
for (var i = 1; i < 29; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/100627PIsa08-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/100627PIsa08-" + i + ".mp3";

    }
    audio.id = 4036 * 1000000 + i;
    audio.albumId = 4036;
    audio.albumName = "以赛亚书系列(八)";
    audio.albumtitle = "以赛亚书系列(八)" + "(" + (i) + ")";
...
fs.writeFile("./ysys_8__songs.json", JSON.stringify(audios, null, '\t'));
