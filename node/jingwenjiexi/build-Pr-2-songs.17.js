...
for (var i = 1; i < 13; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2012Pr02-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2012Pr02-" + i + ".mp3";

    }
    audio.id = 4022 * 1000000 + i;
    audio.albumId = 4022;
    audio.albumName = "箴言系列(二)";
    audio.albumtitle = "箴言系列(二)" + "(" + (i) + ")";
...
fs.writeFile("./pr_2__songs.json", JSON.stringify(audios, null, '\t'));
