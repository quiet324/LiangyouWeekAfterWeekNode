...
for (var i = 1; i < 14; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2013Pr05-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2013Pr05-" + i + ".mp3";

    }
    audio.id = 4025 * 1000000 + i;
    audio.albumId = 4025;
    audio.albumName = "箴言系列(五)";
    audio.albumtitle = "箴言系列(五)" + "(" + (i) + ")";
...
fs.writeFile("./pr_5__songs.json", JSON.stringify(audios, null, '\t'));
