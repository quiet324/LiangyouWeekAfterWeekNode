...
for (var i = 1; i < 13; i++) {
    ...
    if (i < 10) {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2012Mt04-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2012Mt04-" + i + ".mp3";

    }
    audio.id = 4062 * 1000000 + i;
    audio.albumId = 4062;
    audio.albumName = "马太福音系列(四)";
    audio.albumtitle = "马太福音系列(四)" + "(" + (i) + ")";
...
fs.writeFile("./mt_4_songs.json", JSON.stringify(audios, null, '\t'));
