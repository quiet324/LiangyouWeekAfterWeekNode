...
for (var i = 1; i < 13; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2013BibChr06-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2013BibChr06-" + i + ".mp3";

    }
    audio.id = 4005 * 1000000 + i;
    audio.albumId = 4005;
    audio.albumName = "创世记人物(六)";
    audio.albumtitle = "创世记人物(六)" + "(" + (i) + ")";
...
fs.writeFile("./gen_6__songs.json", JSON.stringify(audios, null, '\t'));
