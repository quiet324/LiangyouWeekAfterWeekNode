...
for (var i = 1; i < 17; i++) {
    ...
    if (i < 3) {
        audio.path = "http://www.haomuren.org/Upload/NewsAttach/0709" + (29 + i - 1) + "P.mp3";

    } else if (i < 10) {
        audio.path = "http://www.haomuren.org/Upload/NewsAttach/07100" + (i - 2) + "P.mp3";

    } else {
        audio.path = "http://www.haomuren.org/Upload/NewsAttach/0710" + (i - 2) + "P.mp3";

    }
    audio.id = 4066 * 1000000 + i;
    audio.albumId = 4066;
    audio.albumName = "新约背景与马可福音(一)";
    audio.albumtitle = "新约背景与马可福音(一)" + "(" + (i) + ")";
...
fs.writeFile("./mark_1_songs.json", JSON.stringify(audios, null, '\t'));
