...
for (var i = 1; i < 11; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2017eze04-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2017eze04-" + i + ".mp3";

    }
    audio.id = 4049 * 1000000 + i;
    audio.albumId = 4049;
    audio.albumName = "以西结书系列(四)";
    audio.albumtitle = "以西结书系列(四)" + "(" + (i) + ")";
...
fs.writeFile("./eze_4__songs.json", JSON.stringify(audios, null, '\t'));
