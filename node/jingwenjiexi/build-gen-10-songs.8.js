...
for (var i = 1; i < 10; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2014BibChr10-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2014BibChr10-" + i + ".mp3";

    }
    audio.id = 4009 * 1000000 + i;
    audio.albumId = 4009;
    audio.albumName = "创世记人物(十)";
    audio.albumtitle = "创世记人物(十)" + "(" + (i) + ")";
...
fs.writeFile("./gen_10__songs.json", JSON.stringify(audios, null, '\t'));
