...
for (var i = 1; i <= 15; i++) {
    ...
    if (i < 6) {
        audio.path = "http://www.haomuren.org/Upload/NewsAttach/0703" + (27 + i - 1) + "P.mp3";

    } else if (i < 16) {
        audio.path = "http://www.haomuren.org/Upload/NewsAttach/07040" + (i - 5) + "P.mp3";

    } else {
        audio.path = "http://www.haomuren.org/Upload/NewsAttach/0704" + (i - 5) + "P.mp3";

    }
    audio.id = 4126 * 1000000 + i;
    audio.albumId = 4126;
    audio.albumName = "以弗所书系列";
    audio.albumtitle = "以弗所书系列" + "(" + (i) + ")";
...
fs.writeFile("./yfss_songs.json", JSON.stringify(audios, null, '\t'));
