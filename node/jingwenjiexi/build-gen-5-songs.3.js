...
for (var i = 1; i < 9; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2013BibChr05-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2013BibChr05-" + i + ".mp3";

    }
    audio.id = 4004 * 1000000 + i;
    audio.albumId = 4004;
    audio.albumName = "创世记人物(五)";
    audio.albumtitle = "创世记人物(五)" + "(" + (i) + ")";
...
fs.writeFile("./gen_5__songs.json", JSON.stringify(audios, null, '\t'));
