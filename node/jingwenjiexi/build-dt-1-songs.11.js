...
for (var i = 1; i < 11; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2012Dt01-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2012Dt01-" + i + ".mp3";

    }
    audio.id = 4012 * 1000000 + i;
    audio.albumId = 4012;
    audio.albumName = "申命记系列(一)";
    audio.albumtitle = "申命记系列(一)" + "(" + (i) + ")";
...
fs.writeFile("./dt_1__songs.json", JSON.stringify(audios, null, '\t'));
