...
for (var i = 1; i < 13; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2012Pr01-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2012Pr01-" + i + ".mp3";

    }
    audio.id = 4021 * 1000000 + i;
    audio.albumId = 4021;
    audio.albumName = "箴言系列(一)";
    audio.albumtitle = "箴言系列(一)" + "(" + (i) + ")";
...
fs.writeFile("./pr_1__songs.json", JSON.stringify(audios, null, '\t'));
