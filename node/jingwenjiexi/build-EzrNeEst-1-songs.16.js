...
for (var i = 1; i < 11; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2013EzrNeEst01-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2013EzrNeEst01-" + i + ".mp3";

    }
    audio.id = 4017 * 1000000 + i;
    audio.albumId = 4017;
    audio.albumName = "以斯拉尼希米以斯帖(一)";
    audio.albumtitle = "以斯拉尼希米以斯帖(一)" + "(" + (i) + ")";
...
fs.writeFile("./EzrNeEst_1__songs.json", JSON.stringify(audios, null, '\t'));
