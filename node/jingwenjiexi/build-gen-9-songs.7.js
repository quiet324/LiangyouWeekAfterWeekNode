...
for (var i = 1; i < 11; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2014BibChr09-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2014BibChr09-" + i + ".mp3";

    }
    audio.id = 4008 * 1000000 + i;
    audio.albumId = 4008;
    audio.albumName = "创世记人物(九)";
    audio.albumtitle = "创世记人物(九)" + "(" + (i) + ")";
...
fs.writeFile("./gen_9__songs.json", JSON.stringify(audios, null, '\t'));
