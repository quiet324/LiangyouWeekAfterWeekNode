...
for (var i = 1; i < 11; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2013BibChr04-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2013BibChr04-" + i + ".mp3";

    }
    audio.id = 4003 * 1000000 + i;
    audio.albumId = 4003;
    audio.albumName = "创世记人物(四)";
    audio.albumtitle = "创世记人物(四)" + "(" + (i) + ")";
...
fs.writeFile("./gen_4__songs.json", JSON.stringify(audios, null, '\t'));
