...
for (var i = 1; i < 21; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2013Isa14-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2013Isa14-" + i + ".mp3";

    }
    audio.id = 4042 * 1000000 + i;
    audio.albumId = 4042;
    audio.albumName = "以赛亚书系列(十四)";
    audio.albumtitle = "以赛亚书系列(十四)" + "(" + (i) + ")";
...
fs.writeFile("./ysys_14__songs.json", JSON.stringify(audios, null, '\t'));
