...
for (var i = 1; i < 22; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/110629PIsa09-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/110629PIsa09-" + i + ".mp3";

    }
    audio.id = 4037 * 1000000 + i;
    audio.albumId = 4037;
    audio.albumName = "以赛亚书系列(九)";
    audio.albumtitle = "以赛亚书系列(九)" + "(" + (i) + ")";
...
fs.writeFile("./ysys_9__songs.json", JSON.stringify(audios, null, '\t'));
