...
for (var i = 1; i < 13; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2013Ge01-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2013Ge01-" + i + ".mp3";

    }
    audio.id = 4000 * 1000000 + i;
    audio.albumId = 4000;
    audio.albumName = "创世记人物(一)";
    audio.albumtitle = "创世记人物(一)" + "(" + (i) + ")";
...
fs.writeFile("./gen_1__songs.json", JSON.stringify(audios, null, '\t'));
