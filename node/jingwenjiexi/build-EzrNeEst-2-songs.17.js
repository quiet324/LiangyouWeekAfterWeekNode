...
for (var i = 1; i < 13; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2013EzrNeEst02-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2013EzrNeEst02-" + i + ".mp3";

    }
    audio.id = 4018 * 1000000 + i;
    audio.albumId = 4018;
    audio.albumName = "以斯拉尼希米以斯帖(二)";
    audio.albumtitle = "以斯拉尼希米以斯帖(二)" + "(" + (i) + ")";
...
fs.writeFile("./EzrNeEst_2__songs.json", JSON.stringify(audios, null, '\t'));
