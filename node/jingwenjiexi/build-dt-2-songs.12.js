...
for (var i = 1; i < 11; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2012Dt02-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2012Dt02-" + i + ".mp3";

    }
    audio.id = 4013 * 1000000 + i;
    audio.albumId = 4013;
    audio.albumName = "申命记系列(二)";
    audio.albumtitle = "申命记系列(二)" + "(" + (i) + ")";
...
fs.writeFile("./dt_2__songs.json", JSON.stringify(audios, null, '\t'));
