...
for (var i = 1; i < 11; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2014BibChr12-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2014BibChr12-" + i + ".mp3";

    }
    audio.id = 4011 * 1000000 + i;
    audio.albumId = 4011;
    audio.albumName = "创世记人物(十二)";
    audio.albumtitle = "创世记人物(十二)" + "(" + (i) + ")";
...
fs.writeFile("./gen_12__songs.json", JSON.stringify(audios, null, '\t'));
