...
for (var i = 1; i < 13; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2013Ge03-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2013Ge03-" + i + ".mp3";

    }
    audio.id = 4002 * 1000000 + i;
    audio.albumId = 4002;
    audio.albumName = "创世记人物(三)";
    audio.albumtitle = "创世记人物(三)" + "(" + (i) + ")";
...
fs.writeFile("./gen_3__songs.json", JSON.stringify(audios, null, '\t'));
