...
for (var i = 1; i < 14; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2012Isa13-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2012Isa13-" + i + ".mp3";

    }
    audio.id = 4041 * 1000000 + i;
    audio.albumId = 4041;
    audio.albumName = "以赛亚书系列(十三)";
    audio.albumtitle = "以赛亚书系列(十三)" + "(" + (i) + ")";
...
fs.writeFile("./ysys_13__songs.json", JSON.stringify(audios, null, '\t'));
