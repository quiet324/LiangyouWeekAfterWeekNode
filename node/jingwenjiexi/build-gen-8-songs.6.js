...
for (var i = 1; i < 15; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2014BibChr08-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2014BibChr08-" + i + ".mp3";

    }
    audio.id = 4007 * 1000000 + i;
    audio.albumId = 4007;
    audio.albumName = "创世记人物(八)";
    audio.albumtitle = "创世记人物(八)" + "(" + (i) + ")";
...
fs.writeFile("./gen_8__songs.json", JSON.stringify(audios, null, '\t'));
