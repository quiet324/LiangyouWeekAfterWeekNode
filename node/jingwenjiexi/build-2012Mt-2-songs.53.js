...
for (var i = 1; i < 13; i++) {
    ...
    if (i < 10) {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2012MT02-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2012MT02-" + i + ".mp3";

    }
    audio.id = 4060 * 1000000 + i;
    audio.albumId = 4060;
    audio.albumName = "马太福音系列(二)";
    audio.albumtitle = "马太福音系列(二)" + "(" + (i) + ")";
...
fs.writeFile("./mt_2_songs.json", JSON.stringify(audios, null, '\t'));
