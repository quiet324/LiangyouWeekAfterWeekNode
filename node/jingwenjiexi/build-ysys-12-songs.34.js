...
for (var i = 1; i < 13; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2011Isa12-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2011Isa12-" + i + ".mp3";

    }
    audio.id = 4040 * 1000000 + i;
    audio.albumId = 4040;
    audio.albumName = "以赛亚书系列(十二)";
    audio.albumtitle = "以赛亚书系列(十二)" + "(" + (i) + ")";
...
fs.writeFile("./ysys_12__songs.json", JSON.stringify(audios, null, '\t'));
