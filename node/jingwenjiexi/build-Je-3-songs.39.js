...
for (var i = 1; i < 11; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2012Jer03-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2012Jer03-" + i + ".mp3";

    }
    audio.id = 4045 * 1000000 + i;
    audio.albumId = 4045;
    audio.albumName = "耶利米书系列(三)";
    audio.albumtitle = "耶利米书系列(三)" + "(" + (i) + ")";
...
fs.writeFile("./Jer_3__songs.json", JSON.stringify(audios, null, '\t'));
