...
for (var i = 1; i < 12; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2011Isa11-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2011Isa11-" + i + ".mp3";

    }
    audio.id = 4039 * 1000000 + i;
    audio.albumId = 4039;
    audio.albumName = "以赛亚书系列(十一)";
    audio.albumtitle = "以赛亚书系列(十一)" + "(" + (i) + ")";
...
fs.writeFile("./ysys_11__songs.json", JSON.stringify(audios, null, '\t'));
