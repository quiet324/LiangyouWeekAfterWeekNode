...
for (var i = 1; i < 11; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2015Jdg02-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2015Jdg02-" + i + ".mp3";

    }
    audio.id = 4016 * 1000000 + i;
    audio.albumId = 4016;
    audio.albumName = "士师记系列(二)";
    audio.albumtitle = "士师记系列(二)" + "(" + (i) + ")";
...
fs.writeFile("./jdg_2__songs.json", JSON.stringify(audios, null, '\t'));
