...
for (var i = 1; i < 15; i++) {
    ...
    if (i < 3) {
        audio.path = "http://www.haomuren.org/Upload/NewsAttach/0711" + (29 + i - 1) + "P.mp3";

    } else if (i < 10) {
        audio.path = "http://www.haomuren.org/Upload/NewsAttach/07120" + (i - 2) + "P.mp3";

    } else {
        audio.path = "http://www.haomuren.org/Upload/NewsAttach/0712" + (i - 2) + "P.mp3";

    }
    audio.id = 4067 * 1000000 + i;
    audio.albumId = 4067;
    audio.albumName = "新约背景与马可福音(二)";
    audio.albumtitle = "新约背景与马可福音(二)" + "(" + (i) + ")";
...
fs.writeFile("./mark_2_songs.json", JSON.stringify(audios, null, '\t'));
