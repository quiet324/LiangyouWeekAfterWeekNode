...
for (var i = 1; i < 17; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2011Isa10-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2011Isa10-" + i + ".mp3";

    }
    audio.id = 4038 * 1000000 + i;
    audio.albumId = 4038;
    audio.albumName = "以赛亚书系列(十)";
    audio.albumtitle = "以赛亚书系列(十)" + "(" + (i) + ")";
...
fs.writeFile("./ysys_10__songs.json", JSON.stringify(audios, null, '\t'));
