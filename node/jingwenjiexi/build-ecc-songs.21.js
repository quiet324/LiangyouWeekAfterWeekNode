...
for (var i = 1; i < 15; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2015Ecc-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2015Ecc-" + i + ".mp3";

    }
    audio.id = 4026 * 1000000 + i;
    audio.albumId = 4026;
    audio.albumName = "传道书赏析";
    audio.albumtitle = "传道书赏析" + "(" + (i) + ")";
...
fs.writeFile("./ecc__songs.json", JSON.stringify(audios, null, '\t'));
