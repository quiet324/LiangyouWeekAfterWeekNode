...
for (var i = 1; i < 13; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2013Pr04-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2013Pr04-" + i + ".mp3";

    }
    audio.id = 4024 * 1000000 + i;
    audio.albumId = 4024;
    audio.albumName = "箴言系列(四)";
    audio.albumtitle = "箴言系列(四)" + "(" + (i) + ")";
...
fs.writeFile("./pr_4__songs.json", JSON.stringify(audios, null, '\t'));
