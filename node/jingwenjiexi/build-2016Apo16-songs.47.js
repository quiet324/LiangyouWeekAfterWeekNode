...
for (var i = 1; i < 17; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2016Apo0" + i + "_Rev.mp3";
    } else if (i === 10) {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2016Apo" + i + "_Rev.mp3";
    } else {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2016Apo" + i + "_Dan.mp3";

    }
    audio.id = 4053 * 1000000 + i;
    audio.albumId = 4053;
    audio.albumName = "启示文学讲座";
    audio.albumtitle = "启示文学讲座" + "(" + (i) + ")";
...
fs.writeFile("./apo__songs.json", JSON.stringify(audios, null, '\t'));
