...
for (var i = 1; i < 31; i++) {
    ...
    if (i < 3) {
        audio.path = "http://www.haomuren.org/Upload/NewsAttach/0708" + (30 + i - 1) + "P.mp3";

    } else if (i < 10) {
        audio.path = "http://www.haomuren.org/Upload/NewsAttach/07090" + (i - 2) + "P.mp3";

    } else {
        audio.path = "http://www.haomuren.org/Upload/NewsAttach/0709" + (i - 2) + "P.mp3";

    }
    audio.id = 4077 * 1000000 + i;
    audio.albumId = 4077;
    audio.albumName = "约翰福音系列";
    audio.albumtitle = "约翰福音系列" + "(" + (i) + ")";
...
fs.writeFile("./john_new_songs.json", JSON.stringify(audios, null, '\t'));
