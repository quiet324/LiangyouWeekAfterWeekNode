...
for (var i = 1; i < 13; i++) {
    ...
    if (i < 10) {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2013Mt07-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2013Mt07-" + i + ".mp3";

    }
    audio.id = 4065 * 1000000 + i;
    audio.albumId = 4065;
    audio.albumName = "马太福音系列(七)";
    audio.albumtitle = "马太福音系列(七)" + "(" + (i) + ")";
...
fs.writeFile("./mt_7_songs.json", JSON.stringify(audios, null, '\t'));
