...
for (var i = 1; i < 25; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/100105PIsa07-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/100105PIsa07-" + i + ".mp3";

    }
    audio.id = 4035 * 1000000 + i;
    audio.albumId = 4035;
    audio.albumName = "以赛亚书系列(七)";
    audio.albumtitle = "以赛亚书系列(七)" + "(" + (i) + ")";
...
fs.writeFile("./ysys_7__songs.json", JSON.stringify(audios, null, '\t'));
