...
for (var i = 1; i < 25; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/080417PIsa3-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/080417PIsa3-" + i + ".mp3";

    }
    audio.id = 4031 * 1000000 + i;
    audio.albumId = 4031;
    audio.albumName = "以赛亚书系列(三)";
    audio.albumtitle = "以赛亚书系列(三)" + "(" + (i) + ")";
...
fs.writeFile("./ysys_3__songs.json", JSON.stringify(audios, null, '\t'));
