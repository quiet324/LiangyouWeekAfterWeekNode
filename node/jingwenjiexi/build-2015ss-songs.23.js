...
for (var i = 1; i < 15; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2014Ss01-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2014Ss01-" + i + ".mp3";

    }
    audio.id = 4028 * 1000000 + i;
    audio.albumId = 4028;
    audio.albumName = "雅歌赏析";
    audio.albumtitle = "雅歌赏析" + "(" + (i) + ")";
...
fs.writeFile("./2014_ss__songs.json", JSON.stringify(audios, null, '\t'));
