...
for (var i = 1; i < 21; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/081002PIsa4-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/081002PIsa4-" + i + ".mp3";

    }
    audio.id = 4032 * 1000000 + i;
    audio.albumId = 4032;
    audio.albumName = "以赛亚书系列(四)";
    audio.albumtitle = "以赛亚书系列(四)" + "(" + (i) + ")";
...
fs.writeFile("./ysys_4__songs.json", JSON.stringify(audios, null, '\t'));
