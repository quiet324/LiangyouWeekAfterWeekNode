...
for (var i = 1; i < 13; i++) {
    ...
    if (i < 10) {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2012Mt03-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2012Mt03-" + i + ".mp3";

    }
    audio.id = 4061 * 1000000 + i;
    audio.albumId = 4061;
    audio.albumName = "马太福音系列(三)";
    audio.albumtitle = "马太福音系列(三)" + "(" + (i) + ")";
...
fs.writeFile("./mt_3_songs.json", JSON.stringify(audios, null, '\t'));
