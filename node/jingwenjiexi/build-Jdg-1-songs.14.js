...
for (var i = 1; i < 11; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2015Jdg01-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2015Jdg01-" + i + ".mp3";

    }
    audio.id = 4015 * 1000000 + i;
    audio.albumId = 4015;
    audio.albumName = "士师记系列(一)";
    audio.albumtitle = "士师记系列(一)" + "(" + (i) + ")";
...
fs.writeFile("./jdg_1__songs.json", JSON.stringify(audios, null, '\t'));
