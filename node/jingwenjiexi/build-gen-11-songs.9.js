...
for (var i = 1; i < 10; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2014BibChr11-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2014BibChr11-" + i + ".mp3";

    }
    audio.id = 4010 * 1000000 + i;
    audio.albumId = 4010;
    audio.albumName = "创世记人物(十一)";
    audio.albumtitle = "创世记人物(十一)" + "(" + (i) + ")";
...
fs.writeFile("./gen_11__songs.json", JSON.stringify(audios, null, '\t'));
