...
for (var i = 1; i < 19; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/090323PIsa05-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/090323PIsa05-" + i + ".mp3";

    }
    audio.id = 4033 * 1000000 + i;
    audio.albumId = 4033;
    audio.albumName = "以赛亚书系列(五)";
    audio.albumtitle = "以赛亚书系列(五)" + "(" + (i) + ")";
...
fs.writeFile("./ysys_5__songs.json", JSON.stringify(audios, null, '\t'));
