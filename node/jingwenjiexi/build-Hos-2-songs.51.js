...
for (var i = 1; i < 11; i++) {
    ...
    if (i < 10) {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2013Hos03-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2013Hos03-" + i + ".mp3";

    }
    audio.id = 4058 * 1000000 + i;
    audio.albumId = 4058;
    audio.albumName = "何西阿书系列(三)";
    audio.albumtitle = "何西阿书系列(三)" + "(" + (i) + ")";
...
fs.writeFile("./hos_3_songs.json", JSON.stringify(audios, null, '\t'));
