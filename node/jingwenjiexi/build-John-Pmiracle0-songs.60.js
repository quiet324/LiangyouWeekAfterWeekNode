...
for (var i = 1; i <= 13; i++) {
    ...
    if (i < 10) {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/080718Pmiracle0-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/080718Pmiracle0-" + i + ".mp3";

    }
    audio.id = 4078 * 1000000 + i;
    audio.albumId = 4078;
    audio.albumName = "约翰福音中的神迹";
    audio.albumtitle = "约翰福音中的神迹" + "(" + (i) + ")";
...
fs.writeFile("./john_Pmiracle_songs.json", JSON.stringify(audios, null, '\t'));
