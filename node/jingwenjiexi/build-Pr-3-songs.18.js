...
for (var i = 1; i < 13; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2012Pr03-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2012Pr03-" + i + ".mp3";

    }
    audio.id = 4023 * 1000000 + i;
    audio.albumId = 4023;
    audio.albumName = "箴言系列(三)";
    audio.albumtitle = "箴言系列(三)" + "(" + (i) + ")";
...
fs.writeFile("./pr_3__songs.json", JSON.stringify(audios, null, '\t'));
