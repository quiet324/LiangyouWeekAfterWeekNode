...
for (var i = 1; i < 13; i++) {
    ...
    if (i < 10) {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2012Mt01-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2012Mt01-" + i + ".mp3";

    }
    audio.id = 4059 * 1000000 + i;
    audio.albumId = 4059;
    audio.albumName = "马太福音系列(一)";
    audio.albumtitle = "马太福音系列(一)" + "(" + (i) + ")";
...
fs.writeFile("./mt_1_songs.json", JSON.stringify(audios, null, '\t'));
