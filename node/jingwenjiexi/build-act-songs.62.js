...
for (var i = 1; i <= 16; i++) {
    ...
    if (i < 10) {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2011Acts2-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2011Acts2-" + i + ".mp3";

    }
    audio.id = 4105 * 1000000 + i;
    audio.albumId = 4105;
    audio.albumName = "使徒行传系列(二)";
    audio.albumtitle = "使徒行传系列(二)" + "(" + (i) + ")";
...
fs.writeFile("./act_2_songs.json", JSON.stringify(audios, null, '\t'));
