...
for (var i = 1; i < 11; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2017eze03-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2017eze03-" + i + ".mp3";

    }
    audio.id = 4048 * 1000000 + i;
    audio.albumId = 4048;
    audio.albumName = "以西结书系列(三)";
    audio.albumtitle = "以西结书系列(三)" + "(" + (i) + ")";
...
fs.writeFile("./eze_3__songs.json", JSON.stringify(audios, null, '\t'));
