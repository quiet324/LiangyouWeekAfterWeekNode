...
for (var i = 1; i < 11; i++) {
    ...
    if (i < 10) {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2012Hos01-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2012Hos01-" + i + ".mp3";

    }
    audio.id = 4056 * 1000000 + i;
    audio.albumId = 4056;
    audio.albumName = "何西阿书系列(一)";
    audio.albumtitle = "何西阿书系列(一)" + "(" + (i) + ")";
...
fs.writeFile("./hos_1_songs.json", JSON.stringify(audios, null, '\t'));
