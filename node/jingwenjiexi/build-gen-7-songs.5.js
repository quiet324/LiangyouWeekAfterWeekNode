...
for (var i = 1; i < 15; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2014BibChr07-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2014BibChr07-" + i + ".mp3";

    }
    audio.id = 4006 * 1000000 + i;
    audio.albumId = 4006;
    audio.albumName = "创世记人物(七)";
    audio.albumtitle = "创世记人物(七)" + "(" + (i) + ")";
...
fs.writeFile("./gen_7__songs.json", JSON.stringify(audios, null, '\t'));
