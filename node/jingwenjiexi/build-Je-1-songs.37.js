...
for (var i = 1; i < 11; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2012Jer01-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2012Jer01-" + i + ".mp3";

    }
    audio.id = 4043 * 1000000 + i;
    audio.albumId = 4043;
    audio.albumName = "耶利米书系列(一)";
    audio.albumtitle = "耶利米书系列(一)" + "(" + (i) + ")";
...
fs.writeFile("./Jer_1__songs.json", JSON.stringify(audios, null, '\t'));
