...
for (var i = 1; i <= 12; i++) {
    ...
    if (i < 10) {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2016JsLf11-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2016JsLf11-" + i + ".mp3";

    }
    audio.id = 4102 * 1000000 + i;
    audio.albumId = 4102;
    audio.albumName = "耶稣生平(十一)";
    audio.albumtitle = "耶稣生平(十一)" + "(" + (i) + ")";
...
fs.writeFile("./Jesus_11_songs.json", JSON.stringify(audios, null, '\t'));
