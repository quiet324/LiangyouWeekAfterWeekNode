...
for (var i = 1; i < 11; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2016eze01-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2016eze01-" + i + ".mp3";

    }
    audio.id = 4046 * 1000000 + i;
    audio.albumId = 4046;
    audio.albumName = "以西结书系列(一)";
    audio.albumtitle = "以西结书系列(一)" + "(" + (i) + ")";
...
fs.writeFile("./eze_1__songs.json", JSON.stringify(audios, null, '\t'));
