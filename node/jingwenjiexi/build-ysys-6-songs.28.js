...
for (var i = 1; i < 24; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/090719PIsa06-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/090719PIsa06-" + i + ".mp3";

    }
    audio.id = 4034 * 1000000 + i;
    audio.albumId = 4034;
    audio.albumName = "以赛亚书系列(六)";
    audio.albumtitle = "以赛亚书系列(六)" + "(" + (i) + ")";
...
fs.writeFile("./ysys_6__songs.json", JSON.stringify(audios, null, '\t'));
