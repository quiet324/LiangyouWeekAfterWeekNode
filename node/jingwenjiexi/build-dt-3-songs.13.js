...
for (var i = 1; i < 11; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2012Dt03-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/message/bibleteaching/2012Dt03-" + i + ".mp3";

    }
    audio.id = 4014 * 1000000 + i;
    audio.albumId = 4014;
    audio.albumName = "申命记系列(三)";
    audio.albumtitle = "申命记系列(三)" + "(" + (i) + ")";
...
fs.writeFile("./dt_3__songs.json", JSON.stringify(audios, null, '\t'));
