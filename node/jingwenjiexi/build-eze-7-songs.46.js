...
for (var i = 1; i < 9; i++) {
    ...
    audio.size = "5M";
    if (i < 10) {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2017eze07-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2017eze07-" + i + ".mp3";

    }
    audio.id = 4052 * 1000000 + i;
    audio.albumId = 4052;
    audio.albumName = "以西结书系列(七)";
    audio.albumtitle = "以西结书系列(七)" + "(" + (i) + ")";
...
fs.writeFile("./eze_7__songs.json", JSON.stringify(audios, null, '\t'));
