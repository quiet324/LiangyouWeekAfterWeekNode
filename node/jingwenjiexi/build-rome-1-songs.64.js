...
for (var i = 1; i <= 12; i++) {
    ...
    if (i < 10) {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2016Romans05-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2016Romans05-" + i + ".mp3";

    }
    audio.id = 4111 * 1000000 + i;
    audio.albumId = 4111;
    audio.albumName = "罗马书系列(五)";
    audio.albumtitle = "罗马书系列(五)" + "(" + (i) + ")";
...
fs.writeFile("./rome_5_songs.json", JSON.stringify(audios, null, '\t'));
