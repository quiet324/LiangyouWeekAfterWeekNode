...
for (var i = 1; i <= 11; i++) {
    ...
    if (i < 10) {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2016Jn06-0" + i + ".mp3";

    } else {
        audio.path = "http://media.haomuren.org/Message/BibleTeaching/2016Jn06-" + i + ".mp3";

    }
    audio.id = 4091 * 1000000 + i;
    audio.albumId = 4091;
    audio.albumName = "约翰福音系列(六)";
    audio.albumtitle = "约翰福音系列(六)" + "(" + (i) + ")";
...
fs.writeFile("./john_new2_13_songs.json", JSON.stringify(audios, null, '\t'));
