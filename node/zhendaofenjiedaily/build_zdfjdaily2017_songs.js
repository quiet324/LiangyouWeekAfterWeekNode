...
var names = JSON.parse(fs.readFileSync('./zdfjdaily2017_names_with_dates.json', 'utf8'));
names.forEach(function(name, indexId) {
    var audio = {};
    audio.duration = 150;
    audio.size = "1M";
    audio.path = "http://audio.lyzhendao.net/biblexpo/daily/" + '/' + name.name;
    audio.id = 6006 * 1000000 + indexId + 1;
    audio.albumId = 6006;
    audio.albumName = "《真道分解每日灵修》 2017 上半年合集";
    audio.albumtitle = "《真道分解每日灵修》 2017 上半年合集" + "(" + (indexId + 1) + ")";
    audio.title = "真道分解每日灵修-" + name.date;
...
fs.writeFile("./zdfj_daily_2017_songs.json", JSON.stringify(audios, null, '\t'));
