...
x('http://txly2.net/cwa', 'tbody tr', [{
    ...
    (function(err, results) {
        ...
        results.forEach(function(audio, arrayIndex) {
            ...
            //     fs.writeFileSync('./cwa/' + fileName, data);
            // });
            audio.duration = 1760;
            audio.size = "14.1M";
            audio.artistId = 56;
            audio.artistName = "天路导向（粤语）";
            audio.id = 56000001 + arrayIndex;

        });

        fs.writeFile("./cwa.json", JSON.stringify(results, null, '\t'));
    });