...
var results = JSON.parse(fs.readFileSync('./bible_panshi_names_with_chapter_counts.json', 'utf8'));
var albums = JSON.parse(fs.readFileSync('./build_bible_panshi_albums.json', 'utf8'));
...
results.forEach(function(bible, indexId) {
    ...
    for (i = 1; i < bible.chapterCount + 1; i++) {
        var fileName;
        if (bible.chapterCount === 1) {
            fileName = bible.name.substring(2) + ".mp3";

        } else if (bible.chapterCount >= 100) {
            if (i < 10) {
                fileName = bible.name.substring(2) + " 00" + i + ".mp3";
            } else if (i < 100) {
                fileName = bible.name.substring(2) + " 0" + i + ".mp3";
            } else {
                fileName = bible.name.substring(2) + " " + i + ".mp3";
            }
        } else {
            if (i < 10) {
                fileName = bible.name.substring(2) + " 0" + i + ".mp3";
        ...
        var file = './Panshi/' + bible.name + '/' + fileName;
...
fs.writeFileSync("./build_bible_panshi_albums_and_songs.json", JSON.stringify(audios, null, '\t'));
