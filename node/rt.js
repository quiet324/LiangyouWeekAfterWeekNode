    ...
    (function(err, results) {
        ...
        results.forEach(function(audio, arrayIndex) {
            ...
            download(audio.downUrl).then(data => {
                fs.writeFileSync('../rt/' + fileName, data);
            });
            audio.duration = 1760;
            audio.size = "14.1M";
            audio.artistId = 11;
            audio.artistName = "今夜心未眠";
            audio.id = 11000001 + arrayIndex;

        });

        fs.writeFile("./rt.json", JSON.stringify(results, null, '\t'));
    });